...
protected:
  ...
  void SetUp() override {
    ...
    // Create the SPA entry document where getPage() reads it via assets_path::web().
    ...
    server->resource["^/page-test$"]["GET"] = [](
      ...
      // Note: This reads the SPA index via assets_path::web(), so the fixture creates it in SetUp().
