...
/**
 * @brief Test that Web UI bind address is preferred for URL hosts.
 */
TEST_F(BindAddressTest, WebUiBindAddressFormatsAsUrlHost) {
  config::sunshine.bind_address = "198.51.100.56";
  config::sunshine.web_ui_bind_address = "127.0.0.1";
  EXPECT_EQ(net::get_bind_address_url_host(), "127.0.0.1");

  config::sunshine.web_ui_bind_address = "2001:db8::2";
  EXPECT_EQ(net::get_bind_address_url_host(), "[2001:db8::2]");
}
