<template>
  ...
  <div id="content" class="container">
    ...
    <div class="card my-4">
      <div class="card-body">
        <h2 id="connected_clients">{{ $t('troubleshooting.connected_clients_title') }}</h2>
        <p>{{ $t('troubleshooting.connected_clients_desc') }}</p>
      </div>
      <ul class="list-group list-group-flush" v-if="connectedSessions && connectedSessions.length > 0">
        <li v-for="session in connectedSessions" :key="session.session_id" class="list-group-item d-flex align-items-center">
          <div class="flex-grow-1">
            {{ session.label }}
            <span v-if="session.paused" class="badge bg-warning text-dark ms-2">{{ $t('troubleshooting.connected_clients_paused') }}</span>
          </div>
          <button class="btn btn-warning btn-sm ms-2" @click="toggleSessionPause(session)">
            {{ session.paused ? $t('troubleshooting.connected_clients_resume') : $t('troubleshooting.connected_clients_pause') }}
          </button>
          <button class="btn btn-danger btn-sm ms-2" @click="disconnectSession(session.session_id)">
